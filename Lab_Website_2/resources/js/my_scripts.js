...
function loadStatsPage(){
		...
		var row;//Keeps track of our row index
		var hwins = 0;
		var hloss = 0;
		var homeCell;
		var oppCell;
		for(row = 2; row < table.rows.length; row++){
				homeCell = parseInt(table.rows[row].cells[2].innerHTML);
				oppCell = parseInt(table.rows[row].cells[3].innerHTML);
				if (homeCell > oppCell){
		...
		document.getElementById("losses").innerHTML = hloss;
}
/*
...
function loadPlayersPage(){
	...
	for (var i = 0; i < players.length; i++){
		insert += '<a class="dropdown-item" href="#" onclick="switchPlayers('+i+')">'+players[i].name+'</a>';
	}
	document.getElementById("player_selector").innerHTML = insert;
}
/*
